...
void gb_system::timer::register_system() {
  this->mSystem.mIoBus->register_entry(
      IO_DIV, make_shared<memory::lambda_memory>(
                  [&](uint16_t pAddr) {
                  ...
                  [&](uint16_t pAddr, uint8_t pValue) {
                    ...
                    if (this->mTac & 0x4) {
                      auto oldBit =
                          this->mClocks & TIMA_TICK_BITS[this->mTac & 0x3];
                      if (oldBit) {
                        this->mTima += 1;
  ...
  this->mSystem.mIoBus->register_entry(
      ...
      make_shared<memory::lambda_memory>(
          ...
          [&](uint16_t pAddr, uint8_t pValue) { this->mTima = pValue; }));
  this->mSystem.mIoBus->register_entry(
      IO_TMA,
      make_shared<memory::lambda_memory>(
          [&](uint16_t pAddr) { return this->mTma; },
          [&](uint16_t pAddr, uint8_t pValue) { this->mTma = pValue; }));
  this->mSystem.mIoBus->register_entry(
      IO_TAC,
      make_shared<memory::lambda_memory>(
