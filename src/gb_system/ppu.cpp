...
void gb_system::ppu::register_system() {
  ...
  // VRAM
  memoryBus->register_entry(0x80, 0x20,
                            make_shared<::memory::lambda_memory>(
                                [&](uint16_t pAddr) {
  ...
  // OAM
  memoryBus->register_entry(
      0xfe, 1,
      make_shared<::memory::lambda_memory>(
          [&](uint16_t pAddr) { return this->mOam[pAddr]; },
          [&](uint16_t pAddr, uint8_t pValue) { this->mOam[pAddr] = pValue; }));

  ioBus->register_entry(IO_LCDC,
                        make_shared<::memory::lambda_memory>(
                            [&](uint16_t pAddr) { return this->mLcdc; },
                            [&](uint16_t pAddr, uint8_t pValue) {
  ...
  ioBus->register_entry(IO_SCY,
                        make_shared<::memory::pointer_memory>(this->mScy));
  ioBus->register_entry(IO_SCX,
                        make_shared<::memory::pointer_memory>(this->mScx));
  ioBus->register_entry(IO_LY,
                        make_shared<::memory::pointer_memory>(this->mLy));
  ioBus->register_entry(IO_LYC,
                        make_shared<::memory::pointer_memory>(this->mLyc));
  ioBus->register_entry(IO_BGP,
                        make_shared<::memory::pointer_memory>(this->mBgp));
  ioBus->register_entry(IO_OBP0,
                        make_shared<::memory::pointer_memory>(this->mObp0));
  ioBus->register_entry(IO_OBP1,
                        make_shared<::memory::pointer_memory>(this->mObp1));
  ioBus->register_entry(IO_WY,
                        make_shared<::memory::pointer_memory>(this->mWy));
  ioBus->register_entry(IO_WX,
                        make_shared<::memory::pointer_memory>(this->mWx));

  // CGB implementation
  if (this->mSystem.mSystemType == system_type::CGB) {
    ioBus->register_entry(
        IO_VBK, make_shared<::memory::lambda_memory>(
                    [&](uint16_t pAddr) { return 0xfe | this->mVramBank; },
                    [&](uint16_t pAddr, uint8_t pValue) {
                      this->mVramBank = pValue & 1;
                    }));
    ioBus->register_entry(IO_BCPS,
                          make_shared<::memory::pointer_memory>(this->mBcps));
    ioBus->register_entry(IO_BCPD,
                          make_shared<::memory::lambda_memory>(
                              [&](uint16_t pAddr) {
    ...
    ioBus->register_entry(IO_OCPS,
                          make_shared<::memory::pointer_memory>(this->mOcps));
    ioBus->register_entry(IO_OCPD,
                          make_shared<::memory::lambda_memory>(
                              [&](uint16_t pAddr) {
...
void gb_system::ppu::handle_mode3_enter() { this->mMode = 3; }
